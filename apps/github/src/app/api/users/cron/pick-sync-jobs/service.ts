...
const pickUsersSyncJob = async (job: SelectSyncJob) => {
...
export const pickUsersSyncJobs = async () => {
  ...
  if (jobs.length === 0) {
    return { message: "Their is no syncJobs with type='users' to pick." };
  }

  return await runSyncJobs(jobs, pickUsersSyncJob, { maxRetry: env.USERS_SYNC_MAX_RETRY });
};
